...
void main() {
  ...
  group('execute_getInfo', () {
    test('execute_getInfo_returnsFormattedDeviceInfo', () async {
      ...
      final result = await tool.execute('{"action": "get_info"}', mockContext);
      expect(result.output, contains('Google'));
      expect(result.output, contains('Pixel 8'));
      expect(result.output, contains('14'));
      expect(result.output, contains('34'));
    });
  ...
  group('execute_battery', () {
    test('execute_battery_returnsFormattedBatteryInfo', () async {
      mockContext.setInvokeResult(json.encode({'level': 85, 'charging': true}));
      final result = await tool.execute('{"action": "battery"}', mockContext);
      expect(result.output, contains('85%'));
      expect(result.output, contains('Charging'));
    ...
    test('execute_batteryNotCharging_showsNotCharging', () async {
      ...
      expect(result.output, contains('Not charging'));
    });
  });
  ...
  group('execute_storage', () {
    test('execute_storage_returnsFormattedStorageInfo', () async {
      ...
      final result = await tool.execute('{"action": "storage"}', mockContext);
      expect(result.output, contains('64.5GB'));
      expect(result.output, contains('128.0GB'));
      expect(result.output, contains('63.5GB'));
  ...
  group('execute_memory', () {
    test('execute_memory_returnsFormattedMemoryInfo', () async {
      ...
      final result = await tool.execute('{"action": "memory"}', mockContext);
      expect(result.output, contains('128MB'));
      expect(result.output, contains('512MB'));
      expect(result.output, contains('64MB'));
      expect(mockContext.methodCalls.first.method, 'getDeviceInfo');
    });
  });
  ...
  group('execute_errorHandling', () {
    ...
    test('execute_unknownAction_returnsError', () async {
      ...
      expect(result.toContent(), contains('get_info'));
    });
